...
t2 <- getGranger(diff(ft$count), diff(svi$wsvi), 
                 'newsVol', 'SVI', c(1, 2, 3, 4, 52, 78))
t21 <- getGranger(diff(ft$count), diff(log(svi$wsvi)), 
                  'newsVol', 'lnSVI', c(1, 2, 3, 4, 52, 78))
#t21 <- getGranger(diff(log(ft$count)), diff(log(svi$wsvi)), 
#                  'newsVol', 'lnSVI', c(1, 2, 3, 4, 52, 78))

t3 <- getGranger(diff(ft$count), diff(wbtc$weeklymean), 
                 'newsVol', 'price', c(1, 2, 3, 4, 52, 78))
t31 <- getGranger(diff(ft$count), diff(log(wbtc$weeklymean)), 
                  'newsVol', 'lnPrice', c(1, 2, 3, 4, 52, 78))

t4 <- getGranger(diff(ft$abn_pos), diff(wbtc$weeklymean), 
                 'abnPos', 'price', c(1, 2, 3, 4, 52, 78))
t41 <- getGranger(diff(ft$abn_pos), diff(log(wbtc$weeklymean)), 
                  'abnPos', 'lnPrice', c(1, 2, 3, 4, 52, 78))

t5 <- getGranger(diff(ft$abn_pos), diff(ft$count), 
                 'abnPos', 'newsVol', c(1, 2, 3, 4, 52, 78))

xx <- rbind(c('Null hypothesis', c(1, 2, 3, 4, 52, 78)), 
            t2, t21, t3, t31, t4, t41, t5)
library('stargazer')
stargazer(xx, type = 'html', out='/Users/vir/Downloads/grangerft.htm')
